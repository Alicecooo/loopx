...
| GH-C59 | status | Done — the shipped CLI budget fixtures now cover malformed active-state todo metadata end to end: agent-scoped `status` and `quota should-run` remain inside their measured hot-path latency budgets and fail closed. `status` reports `todo_status_invalid`; quota reports `status_health_ok=false`, `should_run=false`, and `effective_action=quota_skip`. Default backlog output stays compact while bounded public-safe rows remain available behind `--include-detail agent-todos`. | `python3 examples/control_plane/status-quota-perf-budget-smoke.py`, `python3 examples/control_plane/todo-detail-cold-path-contract-smoke.py`, `python3 -m pytest -q tests/control_plane/test_cli_output_budget.py -k malformed_todo_state`, and `loopx check --scan-path docs/status-data-contract.md --scan-path CONTRIBUTOR_TASKS.md` |
| GH-C61 | cli | Done — `/loopx-global-risks` is the fourth shipped canonical global manager CLI command. It remains read-only, consumes bounded compact status and run-history projections, reports unhealthy status as data, and fails closed on malformed sources or unverifiable agent scope. | `python3 examples/project/global-manager-command-protocol-smoke.py`, `python3 examples/project/global-manager-command-cli-smoke.py`, and `loopx check --scan-path docs/reference/protocols/global-manager-command-v0.md --scan-path CONTRIBUTOR_TASKS.md` |
