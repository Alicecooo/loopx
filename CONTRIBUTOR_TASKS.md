...
| GH-C59 | status | Done — the shipped CLI budget fixtures now cover malformed active-state todo metadata end to end: agent-scoped `status` and `quota should-run` remain inside their measured hot-path latency budgets, report the typed contract failure, and fail closed; default backlog output stays compact while bounded public-safe rows remain available behind `--include-detail agent-todos`. | `python3 examples/control_plane/status-quota-perf-budget-smoke.py`, `python3 examples/control_plane/todo-detail-cold-path-contract-smoke.py`, `python3 -m pytest -q tests/control_plane/test_cli_output_budget.py -k malformed_todo_state`, and `loopx check --scan-path docs/status-data-contract.md --scan-path CONTRIBUTOR_TASKS.md` |
