...
| GH-C59 | status | Done — the shipped CLI budget fixture now covers malformed active-state todo metadata end to end: agent-scoped `status` stays within its hot-path output budget and reports the typed contract error, while `quota should-run` fails closed, keeps the default backlog projection compact, and retains bounded public-safe backlog rows behind `--include-detail agent-todos`. | `python3 examples/control_plane/status-quota-perf-budget-smoke.py`, `python3 examples/control_plane/todo-detail-cold-path-contract-smoke.py`, `python3 -m pytest -q tests/control_plane/test_cli_output_budget.py -k malformed_todo_state`, and `loopx check --scan-path docs/status-data-contract.md --scan-path CONTRIBUTOR_TASKS.md` |
