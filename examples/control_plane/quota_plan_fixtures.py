...
def assert_slot_spend_execute(payload: dict, next_should_run: dict, registry_before: str, registry_after: str) -> None:
    ...
    index_records = [json.loads(line) for line in index_lines if line.strip()]
    quota_records = [
        item for item in index_records if item.get("classification") == "quota_slot_spent"
    ]
    assert quota_records, index_records
    assert any(item.get("agent_id") == SCOPED_AGENT_ID for item in quota_records), index_records
